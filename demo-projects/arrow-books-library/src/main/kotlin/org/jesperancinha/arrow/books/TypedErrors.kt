...
data class Book(val name: String = "client${UUID.randomUUID()}", val isdnNumber: Long)
...
fun Book.isValid(): Either<BookNotFound, Unit> = either {
    ensure(isdnNumber > 0) { BookNotFound("A telephone can only be a positive number!!!!: $isdnNumber") }
}
fun Raise<BookNotFound>.isValid(book: Book): Book {
    ensure(book.isdnNumber > 0) { BookNotFound("Telephone without a valid number: ${book.isdnNumber}") }
    return book
}
